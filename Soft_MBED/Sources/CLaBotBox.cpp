...
void CLaBotBox::Start()
{
    ...
    _rs232_pc_rx.attach(this, &CLaBotBox::IRQ_ReceiveRS232);  	// Callback sur rÃ©ception d'une donnÃ©e sur la RS232
...
void CLaBotBox::CheckReceptionTrame(void)
{
  ...
  if  (m_ELECTROBOT_CDE_MOTEURS.isNewTrame() ) {
    Application.m_moteurs.CommandeVitesse(MOTEUR_1, (signed char)m_ELECTROBOT_CDE_MOTEURS.cde_moteur_1);  // Attention : obligation de mettre le cast explicit en "signed", sinon, la valeur est interprÃ©tÃ©e non signÃ©e
    wait_ms(5);
  ...
  if  (m_ELECTROBOT_CDE_SERVOS_AX.isNewTrame() ) {
    // sous adressage : le champ commande_ax donne le type d'action Ã  rÃ©aliser
  ...
  if  (m_ELECTROBOT_CDE_SERVOS_SD20.isNewTrame() ) {
     //_led2 = !_led2;
    // sous adressage : le champ commande_sd20 donne le type d'action Ã  rÃ©aliser
  ...
  if  (m_COMMANDE_VITESSE_MVT.isNewTrame() ) {
      ...
      //TODO : a quelle variable relier les indices de sportivitÃ© de la trame
